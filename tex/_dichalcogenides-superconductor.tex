...
\footnote{%
  In this chapter we use $d$ for these operators,
  however in the appendices, we use $a$ to match the
...
is given in \cref{s:appendix:intrinsic}.
...
\end{equation}
The first term in \cref{eq:channels}
leads to intervalley pairing,
and last two lead to intravalley pairing.
...
\footnote{%
  ...
  without the intervalley term on the first line.
}
